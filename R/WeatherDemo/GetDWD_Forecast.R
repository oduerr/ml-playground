...
forecast1 <- weather_data[[station_id]]$forecast1
forecast1$temperature / 10
...
extract_temperature_data <- function(weather_data) {
  ...
  wd = weather_data[[station_id]]
  wd$forecast1
  forecast1 <- wd$forecast1
  
  # Convert timestamps (milliseconds to seconds)
  start_time <- as.POSIXct(forecast1$start / 1000, origin = "1970-01-01", tz = "UTC")
  ...
  temperatures <- forecast1$temperature / 10  # Convert to Celsius
  forecast1$sunshine
