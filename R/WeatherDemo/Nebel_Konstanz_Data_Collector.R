...
if (FALSE){
  ...
  KN_pos = selectDWD(id=id, res='hourly',  per='hist')
  KN_pos
...
dim(KN_combined)
KN_combined = filter(KN_combined, WW.Beobachtung > -1)
dim(KN_combined)
...
KN_combined <- KN_combined %>%
  mutate(Nebel = ifelse(grepl("Nebel|Feuchter Dunst|NEBEL", WW_Text) & !(WW.Beobachtung %in% c(76, 77, 78)), 1, 0))

if (HIST){
  str(KN_combined)
  summary(KN_combined$V_N.Bedeckungsgrad.x) 
  summary(KN_combined$SD_SO.Sonnenscheindauer) #In minuten 0-60
  #N_combined$V_TE100.Erdbodentemperatur_100cm #Ab etwa 1980
  summary(KN_combined$V_N.Bedeckungsgrad.x) #0 --> klarer Himmel
  summary(KN_combined$V_S1_HHS.Wolkenhoehe_Schicht1)
  summary(KN_combined$TD.Taupunkttemperatur)
  summary(KN_combined$R1.Niederschlagshoehe)
  
  KN_combined %>%
    #ggplot(aes(x=V_N.Bedeckungsgrad.x)) + geom_histogram()
    ggplot(aes(x=MESS_DATUM, V_S1_HHS.Wolkenhoehe_Schicht1))
  
  
  KN_combined %>% select(
      MESS_DATUM, WW.Beobachtung, WW_Text, Nebel, 
      TT_TU.Lufttemperatur, RF_TU.Relative_Feuchte, SD_SO.Sonnenscheindauer) %>% 
    mutate(Lufttemperatur = TT_TU.Lufttemperatur) %>% View()
}
...
if (FALSE){
  ### Historische Daten
  #hist(abs(KN_combined$TT_TU.Lufttemperatur - KN_combined$TT.Lufttemperatur), 1000, xlim = c(0,4))
  
  
  ww_text_table <- KN_combined %>%
    select(WW.Beobachtung, WW_Text, Nebel) %>%
    distinct()
  View(ww_text_table)
}
...
head(nebel_pro_tag)
nebel_pro_tag %>% 
  ggplot(aes(x=Datum, y=anteil_nebel)) + geom_point() 


save(nebel_pro_tag, file="KN_Recent_Nebel_Pro_Tag.RData")
#save(nebel_pro_tag, file="KN_Historisch_Nebel_Pro_Tag.RData")
